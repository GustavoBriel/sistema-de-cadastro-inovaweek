...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBqdG1wenFhcHNscWt6bHdubnpxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzE3MTI4NDksImV4cCI6MjA0NzI4ODg0OX0.H7OIFQHcvnmGq5y1iaSJYjpVtCnW0WehVcSxZhskars';
...
const LoginScreen = ({ navigation }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Logar" onPress={handleLogin} color="#A0C4FF" />
...
const RegisterScreen = ({ navigation }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Registrar" onPress={handleRegister} color="#A0C4FF" />
    </View>
  );
};
...
const PasswordRecoveryScreen = ({ navigation }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {!showPasswordReset && (
        <Button title="Verificar" onPress={handleVerifyUser} color="#A0C4FF" />
      )}
      {showPasswordReset && (
        <>
          <TextInput
            ...
            secureTextEntry
          />
          <Button title="Redefinir Senha" onPress={handleResetPassword} color="#A0C4FF" />
        </>
      )}
      {error ? <Text style={styles.error}>{error}</Text> : null}
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 16, backgroundColor: '#F1FAEE' },
  title: { fontSize: 24, fontWeight: 'bold', marginBottom: 16, color: '#457B9D' },
  subtitle: { fontSize: 20, marginTop: 16, marginBottom: 8, color: '#1D3557' },
  input: { borderWidth: 1, padding: 10, marginBottom: 10, borderRadius: 5 },
  item: { padding: 10, borderBottomWidth: 1, borderColor: '#ccc' },
  error: { color: '#E63946', marginBottom: 10 },
  success: { color: '#2A9D8F', marginBottom: 10, textAlign: 'center' },
  link: { color: '#1D3557', textAlign: 'center', marginTop: 10 },
});
